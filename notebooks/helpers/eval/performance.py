...
def eval_affect(truths, results, exclude_zero=True):
    # If exclude zero is True, it is only considering ground truth labels
    # We are comparing the True Positive Accuracy when we exclude zeros.
    # When we are not excluding zeros, we are considering the total accuracy.
