...
from helpers.eval.complexity import all_in_one_test
from helpers.eval.robustness import relative_robustness, effective_robustness, single_plot
from helpers.eval.evaluation_metric import eval_mosei_senti_return
from helpers.fusions.MCTN import Seq2Seq, L2_MCTN
...
def train(
    ...
    for ep in range(epoch):
        ...
        print(f'Train Loss: {sum_total_loss:.4f} | Regression Loss: {sum_reg_loss:.4f} | Embedding Loss: {(sum_total_loss - sum_reg_loss):.4f}')
        ...
        with torch.no_grad():
            ...
            print(f'Valid MAE: {mae:.4f} | Valid Accuracy: {Acc1*100:.4f}% | Valid Recall: {Acc2*100:.4f}%')
...
def single_test(model, testdata, max_seq_len=20):
    ...
    with torch.no_grad():
        ...
        Acc1 = eval_results_include[-1]
        Acc2 = eval_results_exclude[-1]
        print(f'Test MAE: {mae:.4f} | Test Accuracy: {Acc1*100:.4f}% | Test Recall: {Acc2*100:.4f}%')
        return {'Acc:': Acc2}
